...
class AgentRunner {
  ...
  public function runAsAgent(string $assistant_id, array $chat_history, array $defaults, string $job_id, bool $verbose_mode = FALSE, array $context = []): ChatOutput {
    ...
    // Guard: solve() may return null for tool-only agents; ensure string.
    ...
    if ($history = $agent->getChatHistory()) {
      // Find the last ASSISTANT message (skip system reminders added by
      // require_usage enforcement, which are role='system' not 'assistant').
      foreach (array_reverse($history) as $entry) {
        if ($entry instanceof ChatMessage && $entry->getRole() === 'assistant') {
          $message = $entry;
          $response = $entry->getText();
          break;
        }
      }
    }

...
class AiAgentEntityWrapper implements PluginInterfacesAiAgentInterface, ConfigAiAgentInterface {
  ...
  /**
   * Original account ID saved before masquerade, for restoring after tool run.
   *
   * @var int|null
   */
  protected ?int $originalAccountId = NULL;
  ...
  public function changeUserPermissions() {
    // Save the original account ID so resetUserPermissions() can restore it.
    $this->originalAccountId = $this->currentUser->id();

    $exclude_users_role = $this->aiAgent->get('exclude_users_role');
    $masquerade_roles = $this->aiAgent->get('masquerade_roles');

    // Nothing to do if no permission changes are configured.
    if (!$exclude_users_role && !$masquerade_roles) {
      return;
    }

    // For masquerade: if the current user is anonymous (uid=0), load the admin
    // user (uid=1) so that addRole() works on a real User entity.
    if ($masquerade_roles && $this->currentUser->isAnonymous()) {
    ...
    // For authenticated users, reload the entity to avoid stale data.
    ...
    // Remove all roles if configured.
    ...
    // Set the modified user as the current user.
    $this->currentUser->setAccount($user);
  }
  ...
  public function resetUserPermissions() {
    // Restore the original account using the saved ID (not the possibly-switched
    // current ID, which may differ when masquerade loaded a different user).
    $restore_id = $this->originalAccountId ?? $this->currentUser->id();
    ...
    /** @var \Drupal\user\Entity\User $user */
    $user = $this->entityTypeManager->getStorage('user')->load($restore_id);
    if ($user) {
